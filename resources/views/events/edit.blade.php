...
@section('title', 'Edit Event - Lamoment')

@section('content')
<div class="container py-5">
    <h1 class="h5 mb-4">Edit Event</h1>
        ...
        <div class="card-body">
            <form action="{{ route('events.update', $event) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="client_id" class="form-select @error('client_id') is-invalid @enderror" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->client_id }}" {{ old('client_id', $event->client_id) == $client->client_id ? 'selected' : '' }}>
                                {{ $client->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="package_id" class="form-select @error('package_id') is-invalid @enderror" required>
                        ...
                        @foreach($packages as $package)
                            <option value="{{ $package->package_id }}" {{ old('package_id', $event->package_id) == $package->package_id ? 'selected' : '' }}>
                                {{ $package->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input name="event_type" value="{{ old('event_type', $event->event_type) }}" class="form-control @error('event_type') is-invalid @enderror" required>
                    @error('event_type') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Actual Date</label>
                    <input name="actual_date" type="date" value="{{ old('actual_date', $event->actual_date) }}" class="form-control">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input name="status" value="{{ old('status', $event->status) }}" class="form-control">
                </div>
